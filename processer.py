...
def Process():
    ...
    for i in df.index:
        liste_dates.append(df["date"][i])
        df["Vindstyrke"] = df.replace({"Vindstyrke": {"Frisk bris": 0, "Bris": 1, "Sterk kuling": 2, "Storm": 3, "Liten storm": 4, "Stiv kuling": 5,
        "Stille/svak vind": 6, "Liten kuling": 7}})
    #gjør listen med datoene om til ett set så det bare blir en av hver dato.
    set_date = set(liste_dates)
    ...
    for ind in df.index: 
        ...
        liste_data.append([df["date"][ind], df["weekday"][ind], df["weekend"][ind], df["red_day"][ind], df["avalanche"][ind], df["DangerLevel"][ind], 
        df["CloudCoverId"][ind], df["Nedbor"][ind], df["Vindstyrke"][ind], df["Temperatur_min"][ind], df["Temperatur_max"][ind], df["AvalProbabilityId_0"][ind], df["AvalCauseId_0"][ind], 
        df["DestructiveSizeId_0"][ind], df["AvalTriggerSimpleId_0"][ind], df["AvalProbabilityId_3"][ind], df["AvalCauseId_3"][ind],
        df["DestructiveSizeId_3"][ind], df["AvalTriggerSimpleId_3"][ind], df["AvalProbabilityId_5"][ind], df["AvalCauseId_5"][ind],
        df["DestructiveSizeId_5"][ind], df["AvalTriggerSimpleId_5"][ind], df["AvalProbabilityId_7"][ind], df["AvalCauseId_7"][ind], 
        df["DestructiveSizeId_7"][ind], df["AvalTriggerSimpleId_7"][ind], df["AvalProbabilityId_10"][ind], df["AvalCauseId_10"][ind], 
        df["DestructiveSizeId_10"][ind], df["AvalTriggerSimpleId_10"][ind], df["AvalProbabilityId_30"][ind], df["AvalCauseId_30"][ind],
        df["DestructiveSizeId_30"][ind], df["AvalTriggerSimpleId_30"][ind], df["AvalProbabilityId_45"][ind], df["AvalCauseId_45"][ind],
        df["DestructiveSizeId_45"][ind], df["AvalTriggerSimpleId_45"][ind], df["AvalProbabilityId_50"][ind], df["AvalCauseId_50"][ind],
        df["DestructiveSizeId_50"][ind], df["AvalTriggerSimpleId_50"][ind]])

    #Prosesserer listen med data til verdier mellom 0-1
    scaler = MinMaxScaler()
    scaler.fit(liste_data)
    processed_data = scaler.transform(liste_data)
